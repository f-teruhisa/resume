# 職務経歴書
## 基本情報
### プロフィール
* 氏名: 福本 晃之（ふくもと てるひさ）
* 性別: 男
* 生年月日: 1992年3月24日
* メールアドレス: [teru_fukumoto@outlook.jp](mailto:teru_fukumoto@outlook.jp)


### 関連リンク
* GitHub: [f-teruhisa](https://github.com/f-teruhisa)
* Qiita: [https://qiita.com/TeruhisaFukumoto](https://qiita.com/TeruhisaFukumoto)
* Zenn: [https://zenn.dev/t_fukumoto](https://zenn.dev/t_fukumoto)
* LinkedIn: [https://www.linkedin.com/in/teruhisafukumoto](https://www.linkedin.com/in/teruhisafukumoto)
* Twitter: [@terry_i_](https://twitter.com/terry_i_)
* Facebook: [https://www.facebook.com/teruhisa.fukumoto](https://www.facebook.com/teruhisa.fukumoto)


## 略歴
兵庫県出身。2015年よりSIer企業の株式会社富士通マーケティング(現: 富士通Japan株式会社)に新卒入社。2018年4月にチャットボットによるマーケティング支援サービスを開発する株式会社Zealsへ、Ruby on RailsおよびPythonのサーバサイド開発エンジニアとして入社。同年8月に同社プロダクトマネージャー(PdM)、2019年3月よりVPoEに就任。エンジニア採用やオンボーディング/評価などの組織マネジメント責任を負う。2019年10月より、ヘルステック企業のメドピア株式会社にサーバサイドエンジニアとして入社。主にRuby on Railsを用いたセルフケアプラットフォームサービスの開発に従事。

## 職務経歴
<table>
  <tr>
   <td><strong>期間</strong></td>
   <td><strong>職務内容・役割</strong></td>
   <td><strong>習得したスキル・経験</strong></td>
  </tr>
  <tr>
   <td>20015年6月~20018年3月</td>
   <td>
    <p>株式会社富士通マーケティング</p>
    <p>中小企業の生産管理/会計/人事労務管理システムの導入・リプレース</p>
    <p>担当：法人営業</p>
    <ul>
      <li>新規顧客の開拓</li>
      <li>既存顧客へのシステム提案</li>
    </td>
    <td>
      <ul>
        <li>ITシステムの提案</li>
        <ul>
          <li>既存システム統合</li>
          <li>中規模システムの導入</li>
        </ul>
      </ul>
      <ul>
        <li>法人営業/クライアント折衝</li>
        <ul>
          <li>新規顧客開拓</li>
          <li>既存顧客対応</li>
        </ul>
      </ul>
      <ul>
        <li>ベンダーコントロール</li>
      </ul>
      <ul>
        <li>トラブル時の対応</li>
      </ul>
      </ul>
    </td>
  </tr>
  <tr>
    <td>20018年4月~2018年8月</td>
    <td>
      <p>株式会社Zeals</p>
      <p>チャットボットを用いたマーケティング支援プラットフォーム “Zeals(旧fanp)”のサービス開発</p>
      <p>担当：サーバサイドエンジニア</p>
      <ul>
        <li>Ruby on Railsを用いた管理画面の機能開発およびリファクタリング</li>
        <li>Pythonを用いたLINE/Facebook Messenger API機能の開発およびリファクタリング</li>
      </ul>
    </td>
    <td>
      <ul>
        <li>自社サービスの開発・運用</li>
      </ul>
      <ul>
        <li>コードレビューをする／される</li>
      </ul>
      <ul>
        <li>Ruby on Rails</li>
        <ul>
          <li>Rspecを用いたテスト記述</li>
        </ul>
      </ul>
      <ul>
        <li>Python</li>
        <ul>
          <li>pytestを用いたテスト記述</li>
        </ul>
      </ul>
      <ul>
        <li>JavaScript, jQuery</li>
      </ul>
      <ul>
        <li>SQL(MySQL)</li>
      </ul>
      <ul>
        <li>git(GitHub)</li>
      </ul>
      <ul>
        <li>CircleCI</li>
      </ul>
      <ul>
        <li>Linux</li>
      </ul>
    </td>
  </tr>
  <tr>
   <td>2018年9月~2019年2月</td>
   <td>
    <p>株式会社Zeals</p>
    <p>チャットボットを用いたマーケティング支援プラットフォーム “Zeals(旧fanp)”のプロダクトマネージャー</p>
    <p>担当：プロダクトマネジメント</p>
    <ul>
      <li>サービスの方向性の意思決定</li>
      <li>開発計画の策定、推進</li>
    </ul>
   </td>
   <td>
    <ul>
      <li>短期/中期の開発計画策定</li>
      <ul>
        <li>開発する/しない機能の決定</li>
        <li>経営陣との調整/合意形成</li>
      </ul>
    </ul>
    <ul>
      <li>機能開発の進捗管理</li>
    </ul>
    <ul>
      <li>開発機能の設計</li>
    </ul>
    <ul>
      <li>サービスKPIの設計/運用</li>
    </ul>
    <ul>
      <li>Redashを用いたデータ分析</li>
    </ul>
   </td>
  </tr>
  <tr>
    <td>2019年3月~2019年9月</td>
    <td>
      <p>株式会社Zeals</p>
      <p>エンジニア採用および10人前後規模の開発組織のマネジメント</p>
      <p>担当：VpoE</p>
      <ul>
        <li>エンジニア採用</li>
        <li>技術広報</li>
        <li>チームマネジメント</li>
      </ul>
    </td>
    <td>
      <ul>
        <li>エンジニア採用</li>
        <ul>
          <li>面談/面接の設計および実施</li>
          <li>JobDescriptionの作成</li>
          <li>スカウト作成/送付</li>
          <li>外国籍エンジニアとの英語でのコミュニケーション（オファーを除いた採用フローでの連絡など）</li>
        </ul>
      </ul>
      <ul>
        <li>技術広報</li>
        <ul>
          <li>LINE / Facebook開発者コミュニティでの登壇・運営</li>
          <li>テックブログ運用</li>
        </ul>
      </ul>
      <ul>
        <li>チームマネジメント</li>
        <ul>
          <li>1on1の実施, メンバーの評価</li>
          <li>オンボーディング設計/運用</li>
          <li>グロース時のチーム構成の設計/改善</li>
        </ul>
      </ul>
    </td>
  </tr>
  <tr>
    <td>2019年10月～</td>
    <td>
      <p>メドピア株式会社/株式会社フィッツプラス(出向)</p>
      <p>担当：サーバサイドエンジニア</p>
      <ul>
        <li>Ruby on Railsを用いたAPI/管理画面の機能開発および運用保守</li>
      </ul>
      <ul>
        <li>既存資産(PHP, Perl, Python等)の運用保守</li>
      </ul>
    </td>
  <td>
    <ul>
      <li>Ruby on Rails</li>
      <ul>
        <li>gemのアップデート(RuboCop 0.93.1→1.6.1 etc... )</li>
        <li>Rubyバージョンアップ(2.6.5→2.7.2)</li>
      </ul>
    </ul>
    <ul>
      <li>Python</li>
      <ul>
        <li>Pythonバージョンアップ(2.7→3.6.4)</li>
      </ul>
    </ul>
    <ul>
      <li>Open API環境の構築</li>
      <ul>
        <li>Swagger UI/Editorの導入</li>
        <li>apisproutによるモック環境構築</li>
        <li>committee-railsによるテスタビリティ向上: <a href="https://zenn.dev/t_fukumoto/articles/576ae8e68f6975" target="_blank" rel="noreferrer noopener">https://zenn.dev/t_fukumoto/articles/576ae8e68f6975</a></li>
      </ul>
    </ul>
    <ul>
      <li>Docker</li>
      <ul>
        <li>開発環境の構築／メンテナンス</li>
      </ul>
    </ul>
    <ul>
      <li>AWS</li>
    </ul>
    <ul>
      <li>JavaScript, jQuery</li>
    </ul>
    <ul>
      <li>CircleCI</li>
    </ul>
    <ul>
      <li>SQL(MySQL, PostgreSQL)</li>
    </ul>
    <ul>
      <li>git(GitHub)</li>
    </ul>
    <ul>
      <li>Linux</li>
    </ul>
   </td>
  </tr>
</table>

## 職務以外での経験・活動
<table>
  <tr>
    <td><strong>コミュニティ</strong></td>
    <td><strong>OSS / 個人プロジェクト</strong></td>
  </tr>
  <tr>
    <td>
      <ul>
        <li>Gotanda.rb(2019年3月~)</li>
        <ul>
          <li>五反田のRubyコミュニティ</li>
          <li>月1回のLT会</li>
          <li>URL: <a href="https://github.com/gotandarb" target="_blank" rel="noreferrer noopener">https://github.com/gotandarb</a></li>
        </ul>
      </ul>
      <ul>
        <li>Gotanda.js(2019年5月~)</li>
        <ul>
          <li>五反田のJavaScriptコミュニティ</li>
          <li>月1回のLT会</li>
          <li>URL: <a href="https://github.com/gotandajs" target="_blank" rel="noreferrer noopener">https://github.com/gotandajs</a></li>
        </ul>
      </ul>
    </td>
    <td>
      <ul>
        <li>個人ポートフォリオ</li>
        <ul>
          <li>使用技術: React, Firebase, Docker, CircleCI, Material UI</li>
          <li>URL: <a href="https://teruhisafukumoto.com/" target="_blank" rel="noreferrer noopener">https://teruhisafukumoto.com/</a></li>
          <li>リポジトリ: <a href="https://github.com/f-teruhisa/fukumoto_portfolio" target="_blank" rel="noreferrer noopener">https://github.com/f-teruhisa/fukumoto_portfolio</a></li>
        </ul>
      </ul>
      <ul>
        <li>ドキュメント翻訳</li>
        <ul>
          <li>Gatsby.js: <a href="https://github.com/gatsbyjs/gatsby-ja">https://github.com/gatsbyjs/gatsby-ja</a></li>
          <li>Next.js: <a href="https://github.com/f-teruhisa/Nextjs-ja-translation-docs">https://github.com/f-teruhisa/Nextjs-ja-translation-docs</a></li>
        </ul>
      </ul>
      <ul>
        <li>企業の株価 Slack通知スクリプト</li>
        <ul>
          <li>使用技術: Python, GCP(Cloud Functions+Cloud Scheduler+ Cloud Pub/Sub), GitHub Actions, Docker</li>
          <li>リポジトリ: <a href="https://github.com/f-teruhisa/stooqifier" target="_blank" rel="noreferrer noopener">https://github.com/f-teruhisa/stooqifier</a></li>
          <li>解説記事: <a href="https://zenn.dev/t_fukumoto/articles/e5e0fe753d5726" target="_blank" rel="noreferrer noopener">https://zenn.dev/t_fukumoto/articles/e5e0fe753d5726</a></li>
        </ul>
      </ul>
    </td>
  </tr>
</table>

## 学歴


<table>
  <tr>
   <td><strong>年</strong>
   </td>
   <td><strong>月</strong>
   </td>
   <td><strong>学歴</strong>
   </td>
  </tr>
  <tr>
   <td>2007</td>
   <td>4</td>
   <td>関西大倉高等学校 入学</td>
  </tr>
  <tr>
   <td>2010</td>
   <td>3</td>
   <td>関西大倉高等学校 卒業</td>
  </tr>
  <tr>
   <td>2011</td>
   <td>4</td>
   <td>近畿大学 経済学部 経済学科 入学</td>
  </tr>
  <tr>
   <td>2015</td>
   <td>3</td>
   <td>近畿大学 経済学部 経済学科 情報コース 卒業</td>
  </tr>
</table>

## 資格等
- 基本情報技術者(2015年取得)
